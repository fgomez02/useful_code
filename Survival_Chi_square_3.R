...
##### Filter sites
##########################################################################################################
setwd("/Users/fgomez/Box\ Sync/LymphomaProject/P2C\ ibrutinib\ study\ data/Survival_analysis/output_2")
outdir2 <- "/Users/fgomez/Box\ Sync/LymphomaProject/P2C\ ibrutinib\ study\ data/Survival_analysis/output_2"
...
rec_pers_out <-as.data.frame(rec_pers[which(rec_pers$V1 >2),])
colnames(rec_pers_out) <-c("gene_name","number_of_mutants")

rec_mgi_out <-as.data.frame(rec_mgi[which(rec_mgi$V1 >2),])
colnames(rec_mgi_out) <- c("gene_name","number_of_mutants")

vars_mgi_out <-as.data.frame(vars_mgi)
recurrent_genes<-as.vector(rec_mgi_out$gene_name)
vars_mgi_out_rec <-vars_mgi_out[which(vars_mgi_out$gene_name %in% recurrent_genes),]

write.table(stats, 'filtering_stats.tsv', quote=F, row.names=F, sep="\t")
write.table(rec_pers_out, 'personalis_recurrence.tsv',row.names=F,  quote=F,sep="\t")
write.table(rec_mgi_out, 'mgi_recurrence_filtered.tsv', row.names=F, quote=F,  sep="\t")
write.table(vars_mgi_out_rec, 'mgi_annotation_recurrence_filtered.tsv', row.names=F, quote=F,  sep="\t")
write.table(vars_mgi_out,'mgi_anotated_filtered.tsv', quote=F, col.names= T,row.names = F, sep="\t")
...
samples <- as.factor(unique(vars_mgi[,"sample"]))
...
for (i in 1:length(test_types)){
  for (j in 1:length(genes)){
    ...
    result=plotKM(gene=gene,samples=samples,mutations=mutationdatalist[[i]],min_mutations=min_mut,clinicaldata=clinicaldata,
...
setwd(outdir2)
#write.table(surv_results, file="SurvivalPvaluesTable_duration.tsv", quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
write.table(surv_results, file="SurvivalPvaluesTable_pfs.tsv", quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
#Output results with raw p-value < defined above
#write.table(surv_results[which(as.numeric(surv_results[,"rawp"])<rawpcutoff),], file="SurvivalPvaluesTable_pvaluecutoff_duration.tsv", quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
write.table(surv_results[which(as.numeric(surv_results[,"rawp"])<rawpcutoff),], file="SurvivalPvaluesTable_pvaluecutoff_pfs.tsv", quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)

####################################################################################################################################
###### Do Chi square analysis
###################################################################################################################################

mutations_mgi_3 <-as.data.frame(mutations_mgi)
mutations_mgi_3$sample <-rownames(mutations_mgi)

clinicaldata_2 <- merge(clinicaldata,mutations_mgi_3,by="sample")
clinicaldata_2 <-clinicaldata_2[,1:11]

marker_names <- colnames(mutations_mgi)
marker_data <-mutations_mgi

prognostic_names=c("bestresp")
prognostic_data=clinicaldata_2[,c("bestresp","sample")]

prognostic_data <- na.omit(prognostic_data)
#attach(mutation_coding_ind_3)
#attach(clinical_indv_mut_status)
...
dir.create("/Users/fgomez/Box\ Sync/LymphomaProject/P2C\ ibrutinib\ study\ data/Survival_analysis/output_2/ChiSquare_results",showWarnings = FALSE)
outdir3 <- "/Users/fgomez/Box\ Sync/LymphomaProject/P2C\ ibrutinib\ study\ data/Survival_analysis/output_2/ChiSquare_results"
setwd(outdir3)
